#include "CircleMoveSequence.hpp"

using namespace eeduro::delta;

CircleMoveSequence::CircleMoveSequence(std::string name, Sequence* caller, DeltaControlSystem& controlSys, Calibration& calibration, DeltaSafetyProperties &properties):
...
int CircleMoveSequence::find(const std::array<int,4> &blocks, int block) {
	for (int i = 0; i < blocks.size(); i++) {
		if (blocks[i] == block)
			return i;
	}
